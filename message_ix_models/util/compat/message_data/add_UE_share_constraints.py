...
def main(
    ...
    data = pd.concat([data_gen, data_ssp]).reset_index().drop("index", axis=1)
    ...
    with scen.transact(""):
        for i in data.index:
            ...
            if pd.isnull(row.total_tec) is True:
                tec_list_total = output.technology.unique().tolist()
            else:
                tec_list_total = eval(row.total_tec)
            ...
            if calibration_year >= scen.firstmodelyear:
                ...
                if type(start_year) == dict and row.commodity in start_year.keys():
                    # Set the year for which activity should be retrieved
                    act_year = years[years.index(start_year[row.commodity]) - 1]
                    fix_year = start_year[row.commodity]
            else:
                par = "historical_activity"
                act_year = calibration_year
                fix_year = scen.firstmodelyear
            act_total = scen.par(
                par,
                filters={
                    "node_loc": nodes,
            ...
            if row.target_value == "baseyear":
                for n in nodes:
                    ...
                    df = pd.DataFrame(
                        {
                            "shares": share_name,
                            "node_share": n,
                            "year_act": [y for y in scen.set("year") if y >= fix_year],
                            "time": "year",
                            "value": val,
                            "unit": "-",
                        }
                    ...
                    # Handle 0 values
                    if row.share_type == "upper":
                        df = df.replace(0.0, 0.000001)
                ...
                for n in nodes:
                    if row.target_value == "TS":
                        ...
                        ts = pd.DataFrame(
                            {
                                "year_act": [y for y in years if y >= act_year],
                    ...
                    if row.share_type == "lower":
                        ...
                        check = baseval < ts.iloc[0].value
                    if check == False:
                        if act_year not in ts.index:
                            ts = pd.concat(
                                [
                                    pd.DataFrame(
                                        {"year": act_year, "value": [baseval]}
                                    ).set_index("year"),
                                    ts,
                                ]
                            )
                        else:
                            ts.loc[act_year, "value"] = baseval
                        ts.loc[
                            years[
                                years.index(act_year)
                                + 1 : years.index(act_year)
                                + period_interpol
                            ],
                            "value",
                        ] = np.nan
                    ...
                    ts = ts.loc[ts.year_act >= fix_year]

                    # Handle 0 values
                    if row.share_type == "upper":
                        ts = ts.replace(0.0, 0.000001)
